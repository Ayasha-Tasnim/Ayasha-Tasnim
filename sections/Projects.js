...
const categories = ["All", "Next.js", "React.js", "JavaScript"];
...
export default function Projects() {
  ...
  const filteredProjects = projects.filter((project) => {
    ...
    // Match categories (handling loose string casing differences)
            ...
            {categories.map((category) => (
              <button
                ...
                className={`relative px-5 py-2.5 rounded-full text-xs font-bold uppercase tracking-wider transition-colors duration-300 z-10 ${
                  activeCategory === category ? "text-white" : "text-slate-400 hover:text-white"
                }`}
              >
                {category}
